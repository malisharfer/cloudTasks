...
it('should return object of shift service type', function () {
    $shift = Shift::factory()->create(['is_weekend' => true, 'task_id' => Task::factory()->create(['kind' => TaskKind::NIGHT->value])]);
    $shiftService = new ShiftService(
        $shift->id,
        $shift->task()->withTrashed()->first()->id,
    ...
    expect($shift)->toBeInstanceOf(App\Services\Shift::class);
    expect($shift)->toEqual($shiftService);
    expect($shift->kind)->toBe(TaskKind::WEEKEND->value);
});
...
it('should return array of constraint service type of constraints that have not expired', function () {
    $constraints = Constraint::factory()->count(3)->create([
    ...
    expect(Helpers::buildConstraints($constraints))->toHaveCount(3);
});
...
it('should return the capacity hold of soldiers paramaters', function () {
    ...
    $result = [
        'regulars' => $shifts->filter(fn ($shift) => $shift->kind === TaskKind::REGULAR->value || $shift->kind === TaskKind::NIGHT->value)->count(),
...
it('should return shifts spaces', function () {
    $shifts = Shift::factory()->count(3)->create(['is_weekend' => false, 'start_date' => '2025-06-02 00:00:00', 'task_id' => Task::factory()->create(['kind' => TaskKind::NIGHT->value])->id]);
    $shifts = $shifts->map(fn ($shift) => Helpers::buildShift($shift));
